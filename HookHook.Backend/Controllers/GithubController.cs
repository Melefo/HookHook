...
namespace HookHook.Backend.Controllers
{
	...
	public class GithubController : ControllerBase
	{
        ...
        public GithubController(MongoService service) =>
            _db = service;

        [Authorize]
        [HttpGet("issue")]
        public async Task<ActionResult<string>> GetCommits(string username, string repo)
        {
            var user = _db.GetUser(HttpContext.User.Identity.Name);
            if (user == null)
                return BadRequest();

            var action = new GithubIssueCreated(username, repo);
